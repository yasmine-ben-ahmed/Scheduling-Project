#ifndef SJF_H  // Header guards to prevent multiple inclusions
#define SJF_H
...
int compareBurstTime(const void *a, const void *b) {
    ...
    // Dereference pointers to access structure members
...
static void show_table_SJF(GtkWidget *widget, gpointer data) {
    ...
    // Create a label for the Shortest Job First Algorithm SRT
    ...
    int prev_completion_time = 0;  // Track completion time of the previous process
    int total_waiting_time = 0, total_turnaround_time = 0;
    int num_processes = 0;

    for (int i = 0; process_data[i].id[0] != '\0'; i++) {
        gtk_list_store_append(list_store, &iter);
        // Calculate completion time: completion time = max(arrival time, previous completion time) + burst time
        ...
        // Update previous completion time for the next iteration
...
for (int i = 0; i < 4; i++) {
    ...
    // Set spacing between columns (adjust as needed)
    ...
    gtk_tree_view_column_set_spacing(column, 40); // Adjust the spacing between headers
