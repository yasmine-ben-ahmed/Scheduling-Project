...
void display_roundrobin_interface(Process tab2[], int output[], int outputIndex, int time_quantum) {
...
/***********************________Process Information In Table________********************/  
    ...
    for (int i = 0; i < outputIndex; i++) {
        ...
        gtk_list_store_set(list_store, &iter,
                           ...
                           3, waitingTime,
                           4, turnaroundTime,
                           5, tab2[output[i] - 1].completion_time,
                           -1);
    }
...
/***********************________Calculate and Display Averages ________********************/ 
    ...
    // Calculate total waiting time and total turnaround time
    for (int i = 0; i < n; i++) {
        totalWaitingTime += tab2[output[i] - 1].return_time - tab2[output[i] - 1].arrive_time - tab2[output[i] - 1].execution_time;
        totalTurnaroundTime += tab2[output[i] - 1].return_time - tab2[output[i] - 1].arrive_time;
    }
